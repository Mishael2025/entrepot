...
window.updateHistoryDisplay = function () {
  ...
  history.forEach(entry => {
    ...
    row.innerHTML = `
      ...
      <td>${entry.details ? `DLC : ${entry.details}` : "—"}</td>
    `;
...
window.addToHistory = function (action, nom, date, details, utilisateur = SessionManager.get("username") || "inconnu") {
  const history = JSON.parse(localStorage.getItem("history")) || [];

  history.push({ action, nom, date, details, utilisateur });
  ...
  console.log(`📜 Historique mis à jour : ${action} - ${nom} (${date}) par ${utilisateur}${details ? `, Détail : ${details}` : ""}`);
};
